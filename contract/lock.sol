...
interface MasterChef{
     function stakeTokens( address sender ,uint256 _amount) external ;
     function unstakeTokens(address sender, uint256 _amount) external;
     function receiveReward( address _to) external;
}
 ...
 */
contract LockTokenContract is Context, IERC20, IERC20Metadata {
    ...
    event UserLockToken(address User,uint256 InTokenAmount,uint256 OutLpTokenAmount,uint256 LockTokenBlockNumber);
    event UserUnlockToken(address User,uint256 OutTokenAmount,uint256 InLpTokenAmount);
    ...
    constructor(string memory name_, string memory symbol_, IERC20 _stakeToken) {
        ...
        lockTokenBlockNumber[9] = true;
        lockTokenBlockNumber[12] = true;
        lockTokenBlockNumber[15] = true;
        lockTokenBlockNumber[18] = true;
    }
    ...
    function setMastchef(MasterChef _mast)public {
        require(msg.sender == owner," no auth !");
        mast = _mast;
    }
    ...
    function lockTokens( uint256 _amount, uint256 _lockTokenMounth) public returns (uint256 _id) {
        require(_amount > 0, 'token amount is Zero');
        require(lockTokenBlockNumber[_lockTokenMounth],"_lockTokenBlockNumber  is faild!");
        tokenAddress.safeTransferFrom(msg.sender, address(this), _amount);
    
        uint256 _unlockBlock = 0;
        uint256 lpAmount = 0;
        
        _unlockBlock = block.number.add(_lockTokenMounth.mul(864000));
        
        lpAmount = _amount.mul(_lockTokenMounth).div(3);
        mast.stakeTokens(msg.sender,lpAmount);
        ...
        emit UserLockToken(_withdrawalAddress, _amount, lpAmount, _lockTokenMounth);
    ...
    function withdrawTokens(uint256 _id) public {
        ...
        IERC20(this).safeTransferFrom(msg.sender, address(this), lockedToken[_id].outLpTokenAmount);
        _burn(address(this),lockedToken[_id].outLpTokenAmount);
        
        lockedToken[_id].withdrawn = true;
        mast.unstakeTokens(msg.sender,lockedToken[_id].outLpTokenAmount);
        //update balance in address
        ...
        emit UserUnlockToken(msg.sender, lockedToken[_id].tokenAmount,lockedToken[_id].outLpTokenAmount);
    }
